...
- `MAX_TOKEN` - токен MAX.
- `MAX_CHAT_IDS` - один или несколько ID чатов MAX через запятую. Можно записывать в несколько строк; комментарии после `#` игнорируются.
...
Если MAX-чат есть в `TG_CHAT_MAP`, сообщение уйдет в указанный для него Telegram-чат. Такой чат считается отслеживаемым даже без отдельного дубля в `MAX_CHAT_IDS`.
